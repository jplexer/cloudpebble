...
def start_github_auth(request):
    ...
    redirect_uri = settings.PUBLIC_URL.rstrip('/') + '/ide/settings/github/callback'
    return HttpResponseRedirect('https://github.com/login/oauth/authorize?client_id=%s&scope=repo&state=%s&redirect_uri=%s' %
                                (settings.GITHUB_CLIENT_ID, nonce, urllib.parse.quote(redirect_uri, safe='')))
...
def complete_github_auth(request):
    ...
    r = Request('https://github.com/login/oauth/access_token', params, headers={'Accept': 'application/json'})
    ...
    r.add_header("Authorization", "token %s" % user_github.token)
    result = json.loads(urlopen(r).read())
    user_github.username = result['login']
    user_github.avatar = result['avatar_url']
